      MODULE biology_mod
!
!svn $Id: fennel.h 1081 2021-07-24 02:25:06Z arango $
!=======================================================================
...
#endif
#ifdef BURY
            bury_region(i,j) = 0.0_r8  !DM xn11abbur00: 0% burial in Salish Sea
#endif
...
! not be correct based on newer results in Shiozaki etal (2016).
! >> Here we will turn off all dependence on light. <<
! Note that in the code below there is no oxygen dependence when it
! is dark. That seems like a bug to me.
#ifdef OXYGEN
...
#  ifdef OXYGEN
            IF ((cff1*rOxNH4).gt.Bio(i,1,iOxyg)) THEN
              ! If oxygen is so low that cff1 would drive it below
              ! zero then everything goes to denitrification,
              ! but limited so we don't pull NO3 negative.
            ...
            ELSE
              ...
              IF (cff1.gt.NO3loss) THEN
                ! denitrification, but limited so we don't pull NO3 negative
                cff1_an = MIN(Bio(i,1,iNO3_),NO3loss)
                Bio(i,1,iNO3_)=Bio(i,1,iNO3_)-cff1_an
              END IF
            END IF
#   if defined CARBON && defined TALK_NONCONSERV
